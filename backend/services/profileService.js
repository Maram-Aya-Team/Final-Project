...
const validateProfileUpdate = (data) => {
  const errors = [];
  if (data.username && !USERNAME_REGEX.test(data.username)) errors.push('Username: 3-30 chars, lowercase, numbers, underscores only');
  if (data.bio?.length > 300) errors.push('Bio max 300 characters');
  if (data.city && !ALLOWED_CITIES.includes(data.city)) errors.push('Invalid city');
  if (data.socialLinks) {
    Object.keys(data.socialLinks).forEach(k => {
      if (data.socialLinks[k] && !URL_REGEX.test(data.socialLinks[k])) errors.push(`${k} must be a valid URL`);
    });
  }
  return errors;
};
...
const profileService = {
  ...
  async updateProfile(userId, data) {
    ...
    if (data.username) {
      const existing = await User.findOne({ username: data.username.toLowerCase(), _id: { $ne: userId } }).lean();
    ...
    const user = await User.findByIdAndUpdate(userId, { $set: updates }, { new: true, runValidators: true })
      .select(`${SAFE_FIELDS} notificationPrefs privacy`).lean();
  ...
  async getUserPosts({ userId, type = 'all', cursor, limit = 12 }) {
    ...
    const posts = await Post.find(filter).populate('category', 'name icon').sort({ createdAt: -1 }).limit(safeLimit).lean();
    const nextCursor = posts.length === safeLimit ? posts[posts.length - 1].createdAt : null;
  ...
  async refreshStats(userId) {
    ...
    } : { totalPosts: 0, lostPosts: 0, foundPosts: 0, resolvedPosts: 0 };
